...
from app.services.slides_import import build_slides_data_from_pptx
...
async def import_google_slides(
    ...
    slides_data = build_slides_data_from_pptx(
        Path(tmp_path),
        db,
        doc.id,
        Path(__file__).resolve().parents[3] / "uploads" / "assets",
    )
    doc.front_matter = {**(doc.front_matter or {}), "slides_data": slides_data}
    db.commit()

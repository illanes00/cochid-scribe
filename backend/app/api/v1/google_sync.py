...
async def push_to_google(
    ...
    try:
        ...
        # Execute batchUpdate with retry for rate limits (main content)
        if requests:
            execute_with_retry(
        ...
        # Handle header/footer and page margins from front_matter
        front_matter = doc.front_matter or {}
        page_layout = front_matter.get("page_layout", {})
        header_content = page_layout.get("headerContent")
        footer_content = page_layout.get("footerContent")
        page_margins = page_layout.get("margins")

        # Create header/footer segment requests
        hf_requests = HeaderFooterTransform.create_header_footer_requests(
        ...
        if hf_requests:
            # Execute header/footer creation
            hf_result = execute_with_retry(
                docs_service.documents().batchUpdate(
                    documentId=doc.source_id,
                    body={"requests": hf_requests},
                ),
                operation_name="docs.batchUpdate.headerFooter",
            )

            # Get the created header/footer IDs to populate content
            replies = hf_result.get("replies", [])
            for reply in replies:
                if "createHeader" in reply:
                    header_id = reply["createHeader"].get("headerId")
                    if header_id and header_content:
                        content_requests = HeaderFooterTransform.create_header_content_requests(
                            header_id=header_id,
                            content=header_content,
                            variables={"documentTitle": doc.title or ""},
                        )
                        if content_requests:
                            execute_with_retry(
                                docs_service.documents().batchUpdate(
                                ...
                                operation_name="docs.batchUpdate.headerContent",
                            )
                elif "createFooter" in reply:
                    footer_id = reply["createFooter"].get("footerId")
                    if footer_id and footer_content:
                        content_requests = HeaderFooterTransform.create_footer_content_requests(
                            footer_id=footer_id,
                        ...
                        if content_requests:
                            execute_with_retry(
                                ...
                                operation_name="docs.batchUpdate.footerContent",
                            )
...
async def pull_from_google(
    ...
    try:
        ...
        # Extract header/footer and page margins from Google Doc
        header_content, footer_content = HeaderFooterTransform.extract_header_footer_from_doc(google_doc)
        page_margins = HeaderFooterTransform.extract_page_margins_from_doc(google_doc)
        ...
        # Update front_matter with page layout info
        front_matter = doc.front_matter or {}
        page_layout = front_matter.get("page_layout", {})

        if header_content:
            page_layout["headerContent"] = header_content
            page_layout["showHeaderFooter"] = True
        if footer_content:
            page_layout["footerContent"] = footer_content
            page_layout["showHeaderFooter"] = True
        if page_margins:
            page_layout["margins"] = page_margins

        if page_layout:
            front_matter["page_layout"] = page_layout

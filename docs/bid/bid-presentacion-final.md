---
title: Eficiencia y Calidad del Gasto Público en Seguridad Ciudadana en Chile
subtitle: Informe Final
author: Espacio Público para BID
date: Enero 2026
format: revealjs
---

# Eficiencia y Calidad del Gasto Público en Seguridad Ciudadana

## Informe Final

**Espacio Público**
Enero 2026

---

# Estructura de la Presentación

1. **Recordatorio**: Informe de avance
2. **Marco Conceptual**: Calidad del gasto en seguridad
3. **Hallazgos Consolidados**: Cifras finales 2024
4. **Comparación Internacional**: Chile en contexto global
5. **Hacia la Calidad del Gasto**: Síntesis y brechas
6. **Recomendaciones**: 5 líneas de política pública

---

# Lo que ya cubrimos

## Informe de Avance (Noviembre 2025)

- Contexto: seguridad como prioridad ciudadana
- Fenómeno delictual complejo (homicidios, secuestros)
- Contexto institucional (Ministerio de Seguridad)
- Evolución preliminar del gasto 2013-2024
- Preguntas de investigación

**Hoy**: Hallazgos consolidados, marco conceptual y recomendaciones

---

# I. Marco Conceptual

## Calidad del Gasto en Seguridad

---

# ¿Por qué importa la calidad del gasto?

## El problema no es solo cuánto gastamos

La experiencia comparada del BID muestra que:

> Países con esfuerzos similares presentan **resultados muy distintos** en homicidios, victimización y percepción de seguridad

Esto se explica por:
- Rigideces en la asignación
- Debilidades en la gestión
- Ausencia de seguimiento-evaluación

---

# Eficacia, Eficiencia y Calidad

| Concepto | Definición |
|----------|------------|
| **Eficacia** | ¿Logramos los resultados? (reducir victimización, acortar tiempos) |
| **Eficiencia** | ¿Cuántos resultados por peso gastado? |
| **Calidad** | Focalización + equidad territorial + sostenibilidad + transparencia |

**El giro**: De preguntar "¿cuánto?" a preguntar "¿cómo?"

---

# Los Tres Pilares Funcionales

```
┌─────────────────────────────────────────────────────┐
│                SEGURIDAD CIUDADANA                   │
├─────────────────┬─────────────────┬─────────────────┤
│   CONTROL Y     │   PREVENCIÓN    │   REINSERCIÓN   │
│   REACCIÓN      │                 │   SOCIAL        │
├─────────────────┼─────────────────┼─────────────────┤
│ • Policías      │ • Situacional   │ • Educación     │
│ • Fiscalía      │ • Social        │ • Capacitación  │
│ • Tribunales    │ • Territorial   │ • Apoyo psico-  │
│ • Gendarmería   │                 │   social        │
└─────────────────┴─────────────────┴─────────────────┘
```

---

# Pilar 1: Control y Reacción

## El pilar dominante en Chile

- **Policías**: Vigilancia, detención, investigación
- **Ministerio Público**: Persecución penal
- **Tribunales**: Enjuiciamiento
- **Gendarmería**: Cumplimiento de penas

**COFOG**: 7031, 7033, 7034

Concentra **~96%** del gasto en seguridad

---

# Pilar 2: Prevención

## Intervenciones para reducir factores de riesgo

**Prevención situacional**:
- Iluminación, diseño urbano
- Tecnología de vigilancia

**Prevención social**:
- Programas con jóvenes en riesgo
- Mediación comunitaria

**En Chile**: Subsecretaría de Prevención del Delito (parte de 7036)

---

# Pilar 3: Reinserción Social

## Transformar trayectorias delictivas

Programas para personas en el sistema penal:
- Educación intramuros
- Capacitación laboral
- Apoyo psicosocial
- Acompañamiento post-penitenciario

**Mensaje clave**: Los sistemas concentrados solo en control muestran **retornos decrecientes**

---

# Economía del Crimen (Becker, 1968)

## La decisión de delinquir

```
Beneficio esperado del delito  vs.  Costo esperado

       Ganancia × P(éxito)     vs.   Pena × P(detección)
```

**El gasto público incide en**:
- P(detección): capacidades policiales
- Pena efectiva: inversión en justicia
- Oportunidades: prevención
- Trayectorias: reinserción

---

# Eficiencia Asignativa vs. Técnica

| Tipo | Pregunta | Ejemplo |
|------|----------|---------|
| **Asignativa** | ¿Cómo distribuimos entre funciones? | ¿Más a prevención o a prisiones? |
| **Técnica** | ¿Qué resultados produce cada función? | Tiempos de respuesta, esclarecimiento |

**El desafío**: Chile tiene información sobre asignación pero limitada sobre resultados

---

# II. Hallazgos Consolidados

## Cifras Finales 2024

---

# Cifra Principal

## Gasto en Seguridad 2024

# $4,47 billones CLP

- **1,43%** del PIB
- **5,82%** del gasto del Gobierno Central
- **8,13%** excluyendo Protección Social

---

# Composición del Gasto 2024

```
    Policías (44,0%)
    ████████████████████████████████████████████░░░░░░

    Justicia/MP (31,9%)
    ████████████████████████████████░░░░░░░░░░░░░░░░░░

    Prisiones (20,3%)
    ████████████████████░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░

    Otros (3,8%)
    ████░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
```

---

# Distribución por Subfunción

| Componente | % | Monto (billones CLP) |
|------------|---|----------------------|
| Policías (PDI + Carabineros) | 44,0% | $1,97 |
| Justicia + Ministerio Público | 31,9% | $1,43 |
| Prisiones (Gendarmería) | 20,3% | $0,91 |
| Bomberos | 1,33% | $0,06 |
| Otros n.e.p. | 2,48% | $0,11 |
| **I+D** | **0%** | **$0** |

---

# Hallazgo: Ausencia de I+D

## No se clasificó gasto en I+D en seguridad

**Subfunción 7035**: Investigación y desarrollo en orden público y seguridad

```
Chile:     ████░░░░░░░░░░░░░░░░  0%
OCDE:      ████████░░░░░░░░░░░░  ~1-2%
```

Esto sugiere **énfasis débil** en capacidades analíticas, tecnológicas y de evaluación

---

# Evolución 2013-2024

## En pesos reales (CLP 2024)

```
2013  ██████████████████████████████████████  $3,6T
2015  ████████████████████████████████████████  $3,8T
2017  ██████████████████████████████████████████  $4,0T
2019  ████████████████████████████████████████████  $4,3T
2020  ██████████████████████████████████████  $3,7T ← Pandemia
2022  ████████████████████████████████████████  $3,9T
2024  ████████████████████████████████████████████  $4,5T
```

**Normalización post-pandemia**, sin salto estructural

---

# Evolución como % del PIB

```
       2013   2015   2017   2019   2021   2023   2024
       │      │      │      │      │      │      │
2.0% ─┼──────────────────────────────────────────────
       │            ◆──◆
1.8% ─┼──────────────────────────────────────────────
       │      ◆────◆      ◆
1.6% ─┼──◆─────────────────────────────────────◆─────
       │                        ◆────◆────◆
1.4% ─┼──────────────────────────────────────────◆───
       │                              │      │      │
1.2% ─┴──────────────────────────────────────────────
```

2024: **1,43%** (retorno a rango histórico)

---

# Mensaje Fiscal Clave

## No hay "subfinanciamiento" agregado

> El gasto en seguridad pública se mueve dentro de un "vecindario" normal: crece hasta 2018-2019, cae durante pandemia, luego se normaliza sin saltos estructurales permanentes.

**Implicancia directa**:

> El margen de mejora no está en aumentar escala sino en **mejorar desempeño**

---

# Clasificación Económica

## El gasto es intensivo en personal

```
Personal            ████████████████████████████████████  ~70%
Bienes y Servicios  ████████████░░░░░░░░░░░░░░░░░░░░░░░░  ~20%
Inversión           ████░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░  ~8%
Otros               ██░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░  ~2%
```

**Margen de gestión**: B&S e Inversión para capacidades tácticas

---

# Inercia Asignativa

## La mezcla no cambia pese a cambios en el delito

| Año | Policías | Justicia/MP | Prisiones |
|-----|----------|-------------|-----------|
| 2013 | 44% | 32% | 20% |
| 2017 | 44% | 32% | 20% |
| 2024 | 44% | 32% | 20% |

La asignación responde a **trayectorias históricas** más que a evidencia sobre retornos

---

# III. Comparación Internacional

## Chile en Contexto Global

---

# Metodología Comparada

## Gobierno General (GFSM 2014)

| Aspecto | Análisis Nacional | Comparación Internacional |
|---------|-------------------|---------------------------|
| Nivel | Gobierno Central | Gobierno General |
| Fuente | DIPRES | OCDE, FMI |
| Ajuste | CLP 2024 | US$ PPA 2024 |

---

# Gasto como % del PIB

## Chile vs. Referencias (2023)

```
Mediana OCDE        ████████████████████  1,8%
Promedio Mundial    ████████████████████  1,8%
Chile               ████████████████░░░░  1,56%
Promedio LATAM      ████████████████████  1,9%
```

Chile: **posición intermedia**, levemente bajo mediana OCDE

---

# Gasto como % del Gasto Total

## Chile vs. Referencias (2023)

```
Promedio LATAM      ████████████████████████  6,5%
Chile               ████████████████████░░░░  5,93%
Mediana OCDE        ████████████████████░░░░  5,9%
```

**Esfuerzo presupuestario relativo alineado** con estándares internacionales

---

# Gasto Per Cápita PPA

## Chile en dólares ajustados

| Año | Chile | Mediana OCDE |
|-----|-------|--------------|
| 2019 | $594 | ~$800 |
| 2022 | $482 | $841 |
| 2023 | $511 | ~$850 |
| 2024 | $509 | - |

Chile: **sobre LATAM**, pero **bajo Europa y OCDE**

---

# Por Subfunción: Policías

## Posición media internacional

```
Europa Occidental  ████████████████████████████  $350+ PPA
Mediana OCDE       ████████████████████░░░░░░░░  $280 PPA
Chile              ████████████████░░░░░░░░░░░░  $220 PPA
Promedio LATAM     ████████████████░░░░░░░░░░░░  $210 PPA
```

Recuperación paulatina post-2020

---

# Por Subfunción: Justicia/MP

## Levemente bajo referencias

```
Mediana OCDE       ████████████████████░░░░░░░░  $160 PPA
Promedio LATAM     ██████████████████░░░░░░░░░░  $140 PPA
Chile              ████████████████░░░░░░░░░░░░  $120 PPA
```

Baja gradual en la última década

---

# Por Subfunción: Prisiones

## Chile gasta ALTO comparativamente

```
Chile              ████████████████████████████  $105 PPA
Europa Occidental  ████████████████████████░░░░  $95 PPA
Mediana OCDE       ████████████████████░░░░░░░░  $80 PPA
Promedio LATAM     ██████████████░░░░░░░░░░░░░░  $55 PPA
```

Refleja presión sobre **estándares penitenciarios** y ocupación

---

# Por Subfunción: Bomberos

## Muy bajo (modelo voluntariedad)

```
Mediana OCDE       ████████████████████░░░░░░░░  $50 PPA
Europa             ████████████████████████████  $70 PPA
Chile              ████░░░░░░░░░░░░░░░░░░░░░░░░  $8 PPA
```

Gasto público directo mínimo, financiamiento vía subsidios/transferencias

---

# Por Subfunción: I+D

## Chile = 0

```
Países OCDE con I+D   ████████░░░░░░░░░░░░  $5-15 PPA
Chile                 ░░░░░░░░░░░░░░░░░░░░  $0 PPA
```

Frente incipiente de inversión en la mayoría de países, **nulo en Chile**

---

# Síntesis Comparativa

| Subfunción | Posición de Chile |
|------------|-------------------|
| Policías | Media |
| Justicia/MP | Levemente baja |
| **Prisiones** | **Alta** |
| Bomberos | Muy baja |
| **I+D** | **Nula** |

---

# IV. Hacia la Calidad del Gasto

## Síntesis y Brechas

---

# 5 Hallazgos Principales

1. **Esfuerzo relevante y comparable** internacionalmente
2. **Mezcla estable y concentrada** en control (96%)
3. **Gasto intensivo en personal** (70%)
4. **Prisiones alta, I+D nula** en comparación
5. **Brechas de información** limitan evaluación

---

# Hallazgo 1: Esfuerzo Comparable

## No hay subinversión extrema

```
Chile vs. OCDE (% PIB)
OCDE:   ████████████████████  1,8%
Chile:  ████████████████░░░░  1,56%
                         ↑
                  Brecha acotada
```

El problema **no es cuánto** sino **cómo**

---

# Hallazgo 2: Concentración en Control

## ~96% va a policías, justicia y prisiones

```
Control (96%)     ████████████████████████████████████████████████
Prevención (~3%)  ██░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
Reinserción (~1%) █░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
```

Riesgo: **retornos decrecientes** sin equilibrio

---

# Hallazgo 3: Intensivo en Personal

## Margen de gestión acotado

```
Personal:          70%  ████████████████████████████████████████
B&S + Inversión:   28%  ████████████████░░░░░░░░░░░░░░░░░░░░░░░░
```

Sin tocar dotaciones, el espacio está en **insumos, equipamiento e infraestructura**

---

# Hallazgo 4: Desequilibrios en Mezcla

## Alto en prisiones, nulo en I+D

| Alto | Bajo |
|------|------|
| Prisiones (+25% sobre OCDE per cápita) | I+D (0%) |
| | Bomberos (-85%) |

Privación de libertad es **costosa** y de **última ratio**

---

# Hallazgo 5: Brechas de Información

## No se puede vincular gasto con resultados

```
Presupuesto  →  ???  →  COFOG  →  ???  →  Resultados
     │                   │                    │
     └───────────────────┴────────────────────┘
              Sin mapeo público
```

---

# Cuatro Desafíos de Información

1. **Trazabilidad**: No hay diccionario público programas → COFOG
2. **Desagregación**: Faltan desgloses por servicio y territorio
3. **Interoperabilidad**: Registros fragmentados entre instituciones
4. **Evaluación**: Pocas evaluaciones de impacto rigurosas

---

# El Giro Necesario

## De "cuánto gastamos" a "cómo gastamos"

```
ANTES:  ¿Chile gasta poco o mucho en seguridad?

AHORA:  ¿Qué tan bien se usan los recursos?
        ¿Qué ajustes mejorarían el impacto?
```

---

# V. Recomendaciones

## 5 Líneas de Política Pública

---

# Recomendación 1

## Reforzar Prevención y Reinserción

**Sin disminuir abruptamente el control**:

- Recomposición **gradual** del gasto
- Fortalecer Subsecretaría de Prevención del Delito
- Alinear gasto penitenciario con objetivos de **reducción de reincidencia**
- Priorizar programas con **evidencia de efectividad**

---

# Recomendación 2

## Abrir Espacio para I+D

**Establecer línea de gasto en subfunción 7035**:

- Datos y modelos de gestión
- Evaluación de programas
- Desarrollo tecnológico
- Experimentación y pilotos

**Objetivo**: Preparar el sistema para enfrentar delito más sofisticado

---

# Recomendación 3

## Profundizar Interoperabilidad

**Plan de vinculación presupuesto-COFOG-resultados**:

- Publicar diccionario de mapeo programas → subfunciones
- Paneles periódicos: gasto + dotaciones + indicadores
- Desagregación por institución y territorio
- Identificadores comunes entre instituciones

---

# Recomendación 4

## Desarrollar Métricas de Desempeño

**Introducción progresiva de indicadores**:

| Área | Indicadores |
|------|-------------|
| Policías | Tiempos respuesta, esclarecimiento |
| Justicia | Duración causas, tasa condena |
| Prisiones | Hacinamiento, reincidencia |

**Objetivo**: Orientar recursos hacia áreas con mejores resultados

---

# Recomendación 5

## Consolidar Línea Base

**Este informe como punto de partida**:

- Actualización periódica de series
- Módulos de análisis más sofisticados
- Estudios de eficiencia técnica
- Evaluaciones de impacto específicas

**Cerrar brecha** entre diagnóstico fiscal y gestión cotidiana

---

# Síntesis de Recomendaciones

```
┌─────────────────────────────────────────────────────┐
│              CALIDAD SOBRE CANTIDAD                  │
├─────────────────────────────────────────────────────┤
│ 1. Reforzar prevención y reinserción                │
│ 2. Crear espacio para I+D                           │
│ 3. Profundizar interoperabilidad                    │
│ 4. Desarrollar métricas de desempeño                │
│ 5. Consolidar línea base para evaluación            │
└─────────────────────────────────────────────────────┘
```

---

# VI. Cierre

---

# Mensaje Final

> **Dado el nivel de esfuerzo que Chile ya realiza en seguridad, el principal margen de mejora se encuentra en cómo se gastan los recursos y cómo se conectan con resultados.**

Avanzar hacia una seguridad con mayor **calidad del gasto**:
- Es una exigencia fiscal
- Es condición para recuperar confianza ciudadana

---

# Próximos Pasos

1. **Socialización** de hallazgos con autoridades
2. **Piloto de interoperabilidad** con 2-3 instituciones
3. **Diseño de indicadores** de desempeño prioritarios
4. **Actualización anual** de series COFOG
5. **Evaluaciones de impacto** en programas seleccionados

---

# Contacto

## Espacio Público

**Equipo del proyecto**:
- Benjamín García (Director Ejecutivo)
- Eleni Kokkidou (Subdirectora Ejecutiva)
- Patricio Domínguez (Director)
- Mauricio Duce (Director)
- Martín Illanes (Investigador)

---

# ¡Gracias!

## Eficiencia y Calidad del Gasto Público en Seguridad Ciudadana en Chile

**Espacio Público para BID**
Enero 2026

---

# Anexo: Datos Clave

---

# Tabla: Gasto por Función 2024

| Función | Billones CLP | % PIB | % Gasto |
|---------|--------------|-------|---------|
| Protección Social | 25,7 | 8,2% | 33,5% |
| Educación | 9,1 | 2,9% | 11,9% |
| Salud | 8,3 | 2,7% | 10,8% |
| **Seguridad** | **4,5** | **1,4%** | **5,8%** |
| Defensa | 2,8 | 0,9% | 3,7% |

---

# Tabla: Gasto por Subfunción 2024

| Subfunción | Billones CLP | % |
|------------|--------------|---|
| Policías | 1,97 | 44,0% |
| Justicia/MP | 1,43 | 31,9% |
| Prisiones | 0,91 | 20,3% |
| Bomberos | 0,06 | 1,3% |
| N.E.P. | 0,11 | 2,5% |
| I+D | 0,00 | 0,0% |
| **Total** | **4,47** | **100%** |

---

# Gráfico: Evolución del Gasto

```
Billones CLP 2024

4.5 ┤                                          ◆
    │                                    ◆────┘
4.0 ┤                              ◆────┘
    │                    ◆────◆────┘
3.5 ┤              ◆────┘    └────◆
    │        ◆────┘
3.0 ┤  ◆────┘
    │──┘
2.5 ┴────────────────────────────────────────────
    2013  2015  2017  2019  2021  2023  2024
```

---

# Gráfico: Comparación Internacional

```
Gasto per cápita PPA (US$ 2024)

$900 ┤████████████████████████████████████████████████  OCDE
     │
$700 ┤████████████████████████████████████░░░░░░░░░░░░  Europa
     │
$500 ┤████████████████████████░░░░░░░░░░░░░░░░░░░░░░░░  Chile
     │
$300 ┤████████████████░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░  LATAM
     │
$100 ┴────────────────────────────────────────────────
```

---

# Referencias

- DIPRES - Estadísticas COFOG (2013-2024)
- OCDE - Government at a Glance 2025
- FMI - Government Finance Statistics
- BID - Estudios sobre crimen y seguridad en América Latina
- Ministerio Público - Estadísticas de persecución penal
- INE - ENUSC (Encuesta Nacional Urbana de Seguridad Ciudadana)

---

# Fin de la Presentación
